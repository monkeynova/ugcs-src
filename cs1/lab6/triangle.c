...
void Draw_Row(CS1_Window, int, int,int);
...
void Next_Move(int board_in[15],int move,int sign,int depth) {
  ...
  if (depth==2) while(!(key=cs1_inkey()));
  if (key=='q') exit();
...
void Disp_Board(int board[15], int status, int depth) {
  ...
  Draw_Row(mw,5,y,position);
}

void Draw_Row(CS1_Window mw, int t, int y,int position) {
  int s;
  if (t==0) return;
  for (s=0;s<=t;s++) {
    if (board[n]==0) {
